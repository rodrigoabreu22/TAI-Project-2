...
uint32_t FlatFrequencyTable::get(uint32_t symbol) const { checkSymbol(symbol); return 1; }
uint32_t FlatFrequencyTable::getTotal() const { return numSymbols; }
uint32_t FlatFrequencyTable::getLow(uint32_t symbol) const { checkSymbol(symbol); return symbol; }
uint32_t FlatFrequencyTable::getHigh(uint32_t symbol) const { checkSymbol(symbol); return symbol + 1; }
void FlatFrequencyTable::set(uint32_t, uint32_t) { throw std::logic_error("Unsupported operation"); }
void FlatFrequencyTable::increment(uint32_t) { throw std::logic_error("Unsupported operation"); }
